...
st.set_page_config(layout=wide, page_title=Ruta del Café, page_icon=.imgcafe5.png)
...
if hora_actual  time(18, 00)
    st.sidebar.write(f'Mira que horas son, no deberías estar aquí, pero faltan {horas_restantes} horas y {minutos_restantes} minutos para el ☕ de las 1030')
elif hora_actual  primer_cafe
    st.sidebar.write(f{horas_restantes} horas y {minutos_restantes} minutos para el ☕ de las 1030)
    ...
    st.sidebar.write(f{horas_restantes2} horas y {minutos_restantes2} minutos para el ☕ de las 1445)
else
...
LOGO_IMAGE = .imghavas.png

# Texto principal
texto_principal = 'h1 style=text-aligncenterspan style=font-size 40px;☕span uLA RUTA DEL CAFÉuh1'
...
st.markdown(
    f'div class=logo-container{texto_principal}img src=dataimagepng;base64,{encoded_image} class=logo-imgdiv',
    unsafe_allow_html=True
)
...
def gente()
    personas = ['Adrián', 'Álvaro D.', 'Álvaro S.', 'Ana G.', 'Ana M.', 'Bernardo', 'Dani A.', 'Dani S.', 'Inés MG', 'Inés ML', 'Javi B.', 'Javi N.', 'Lucas', 'Lucía', 'María E.', 'María L.', 'Maxi', 'Mercedes', 'Pablo', 'Rafa', 'Rosalía', 'Rubén C.', 'Rubén I.', 'Sergio', 'Víctor' ]   
    try
...
try
    ...
    for persona in gente()
        ...
        if seleccion
            ...
            if persona == 'Adrián'
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Té Rojo 🔴'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index('Sin Lactosa 🆓'))
                extras = col4.text_input(fExtras de {persona})
                if hora_actual  time(14, 00)
                    barrita = col5.selectbox(fTostada de {persona}, tostadas, index=tostadas.index('Cereales 🌾'))
                else
                    barrita = col5.selectbox(fTostada de {persona}, tostadas)               

            elif persona == 'Ana M.' or persona == 'Inés MG' or persona == 'María E.'
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Café ☕'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index('Leche 🥛'))
                extras = col4.text_input(fExtras de {persona}, 'Desnatada') 
                barrita = col5.selectbox(fTostada de {persona}, tostadas)
                ...
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Descafeinado ☕'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index('Leche Almendra 🌰'))
                extras = col4.text_input(fExtras de {persona})
                if hora_actual  time(14, 00)
                    barrita = col5.selectbox(fTostada de {persona}, tostadas, index=tostadas.index('Integral 🥔'))
                else
                    barrita = col5.selectbox(fTostada de {persona}, tostadas)   
                ...
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Café ☕'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index('Solo ❌'))
                extras = col4.text_input(fExtras de {persona}, 'Largo') 
                barrita = col5.selectbox(fTostada de {persona}, tostadas)
                ...
                extras = col4.text_input(fExtras de {persona}, 'Largo') 
                if hora_actual  time(14, 00)
                    barrita = col5.selectbox(fTostada de {persona}, tostadas, index=tostadas.index('Blanco 🥖'))
                else
                    barrita = col5.selectbox(fTostada de {persona}, tostadas)  

            elif persona == 'Dani S.'
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Manzanilla 🍵'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index(''))
                extras = col4.text_input(fExtras de {persona}) 
                barrita = col5.selectbox(fTostada de {persona}, tostadas)

            elif persona == 'Mercedes'
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Cola Cao 🥜'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index('Leche 🥛'))
                extras = col4.text_input(fExtras de {persona}) 
                barrita = col5.selectbox(fTostada de {persona}, tostadas)
             
            elif persona == 'Javi N.'
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas, index=bebidas.index('Zumo 🍊'))
                con_seleccionada = col3.selectbox(f'Con' de {persona}, con, index=con.index(''))
                extras = col4.text_input(fExtras de {persona}) 
                barrita = col5.selectbox(fTostada de {persona}, tostadas)
                ...
                bebida_seleccionada = col2.selectbox(fBebida de {persona}, bebidas)
    ...
    if len(seleccionados)  0
        ...
        pedido_str = 'Hola! Os hago un pedidonn'

        for key, value in conteo.items()
            pedido_str += • {} {}n.format(value, key)
            
        if len(n_tostadas)  0
            pedido_str +='nBarritas de pann'
            for key, value in n_tostadas.items()
                pedido_str += • {} {}n.format(value, key)

        if para_llevar == Sí, el trabajo nos reclama 💻
            pedido_str += 'n(Todos para llevar y con leche templada)n'
        else
            pedido_str += 'n(Todos con leche templada)n'
    ...
    # ---------------------------------------------------------------------------
        ...
        output.append('Hola! Os hago un pedidon')
...
# --------------- solos ----------------------------------------------------------------------------------------
        ...
        if len(n_tostadas)  0
            output.append('nBarritas de pan')
            if cereal  0
        ...
        if para_llevar == Sí, el trabajo nos reclama 💻
            output.append('n(Todos para llevar y con leche templada)')
        else
            output.append('n(Todos con leche templada)')

        output.append('Muchas gracias! 🙂')

        st.code('n'.join(output), language='plaintext')
